...
dlCurve::dlCurve(const short Channel) {
  m_IntType = dlCurveIT_Spline;
...
short dlCurve::SetNullCurve(const short Channel) {
  m_IntendedChannel = Channel;
  if (Channel == dlCurveChannel_Saturation) {
    m_Type            = dlCurveType_Anchor;
    m_NrAnchors       = 3;
    m_XAnchor[0]      = 0.0;
    m_YAnchor[0]      = 0.5;
    m_XAnchor[1]      = 0.5;
    ...
    m_XAnchor[2]      = 1.0;
    m_YAnchor[2]      = 0.5;
  } else if (Channel == dlCurveChannel_a || Channel == dlCurveChannel_b) {
    m_Type            = dlCurveType_Anchor;
    m_NrAnchors       = 3;
    m_XAnchor[0]      = 0.0;
    m_YAnchor[0]      = 0.0;
...
short dlCurve::SetCurveFromAnchors() {

  assert (m_NrAnchors != 0);

  m_Type = dlCurveType_Anchor;

  if (m_IntType == dlCurveIT_Spline) {
    double *ypp = spline_cubic_set(m_NrAnchors,
                                   ...
                                   2, 0.0);
    if (NULL==ypp) {
      dlLogError(dlError_Spline,"Unexpected NULL return at %s line %d\n",
                 __FILE__,__LINE__);
      return -1;
    }
...
short dlCurve::DumpData(const char* FileName,
                        const short Scale) {
...
short dlCurve::WriteCurve(const char *FileName,
                          const char *Header) {
...
short dlCurve::ReadCurve(const char *FileName) {

  // Some safe presets in case things go wrong.
  m_NrAnchors = 0;
  m_IntendedChannel = dlCurveChannel_L;
  m_Type = dlCurveType_Full;
  memset(m_Curve,0,sizeof(m_Curve));


  FILE *InFile = fopen(FileName,"r");
  if (!InFile) {
    //~ dlLogError(dlError_FileOpen,"File %s could not be opened\n",FileName);
    return dlError_FileOpen;
  }
  ...
  do {
    ...
    if (1 == NrKeys) {
      if ((strcmp(Key,"Magic") || strcmp(Value,"FLOSSCurveFile")) &&
        ...
        dlLogError(dlError_FileFormat,
                   "'%s' has wrong format at line %d\n",
                   FileName,
                   LineNr);
        return dlError_FileFormat;
      }
    ...
    } else if (m_Type == dlCurveType_Anchor) {
      ...
      m_NrAnchors++;
    } else if (m_Type == dlCurveType_Full) {
...
short dlCurve::ReadAnchors(const char *FileName) {
...
short dlCurve::SetCurveFromFunction(double(*Function)
...
short dlCurve::Set(dlCurve *Curve) {
  m_Type = Curve->m_Type;
...
dlCurve* dlCurve::ApplyCurve(const dlCurve* Curve,
...
//    entries was zero.
//
double *dlCurve::d3_np_fs ( int n, double a[], double b[] ) {
  int i;
  double *x;
  double xmult;
//
...
//
  ...
  dlMemoryError(x,__FILE__,__LINE__);
...
//
//
double *dlCurve::spline_cubic_set ( int n, double t[], double y[], int ibcbeg,
...
//
  ...
  dlMemoryError(a,__FILE__,__LINE__);
  b = (double *)CALLOC(n,sizeof(double));
  dlMemoryError(b,__FILE__,__LINE__);
//
...
//
  if ( n == 2 && ibcbeg == 0 && ibcend == 0 ) {
    ...
    dlMemoryError(ypp,__FILE__,__LINE__);
...
double dlCurve::spline_cubic_val ( int n, double t[], double tval, double y[],
